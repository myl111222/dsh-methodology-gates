...
// Variant B (0.2.0): the always-on section names NO methodology. All 36 are equal
// and the SCENARIO decides, so what stays resident is the selection mechanism —
// the trigger that makes the choice happen — plus the one rule that cannot be
// satisfied by thinking at all (adversarial review needs another context; ablation
// needs a baseline and a real re-run).
//
// Why the 13-name map was dropped: its membership came from a conference slide
// deck, not from evidence. This plugin's own retrieval tests kept proving that
// non-core entries were the right answer (线上崩了要写复盘 → 无指责复盘, 测试挂了一片
// → 红绿重构, 想推倒重写 → 加尔定律). A privileged subset is a ranking the data
// does not support.
...
const HARD_LINE = '唯一的硬规则：涉及交付 / 完成时，'
  + HARD_GATES.map(wordOf).filter((w) => w !== '').join(' / ')
  + ' 若没有真做（前者必须另开一个没参与实现的上下文，后者必须先有基线、再删一个组件真跑一遍），明说“没做”，不要默认通过。';
const SECTION_TEXT = '方法论闸门（由 dsh-methodology-gates 插件注入）：' + TABLE.length + ' 条方法论同等重要，没有哪条常驻——用哪条由场景决定。\n'
  + '· 开工前、以及每次换阶段（设计 → 实现 → 排查 → 交付），先调 methodology action=route（task=你当前要做的事）：它按每条自己的“什么时候用”与你的任务匹配，从 ' + TABLE.length + ' 条里取本情境的闸门，命中词会标出。\n'
  + '· 说出方法论的名字即启用（流程已在你能力里）；不确定某条怎么用 → action=show <词>；通览 → action=list。\n'
  + '· ' + HARD_LINE;
const TOOL_DESC = '方法论闸门：按情境取用 ' + TABLE.length + ' 条方法论，没有哪条常驻。action=route 用 task（一句话说你当前要做的事）按情境取相关闸门并标出命中词；action=list 看目录（可配 family）；action=show 用 word 取某一条完整用法（什么时候用／动作／产出物／误用／执行机构／来源）。';
