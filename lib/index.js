      ...
      async execute(args) {
        ...
        if (action === 'show') {
          ...
          return r.hit === undefined
            ? { kind: 'show', text: notFound(a.word) }
            : { kind: 'show', text: brief(r.hit, r.loose, r.others) };
        }
        if (action === 'route') return { kind: 'route', text: routeText(a.task) };
        return { kind: 'list', text: catalog(a.family) };
      }
    })));
  }
